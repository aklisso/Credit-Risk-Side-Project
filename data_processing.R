...
full_train = read.csv("credit_train.csv")
...
full_train = full_train %>% mutate (across(all_of(cat_vars), as.factor))
...
train_fs_ig = train %>% select(any_of(topfeatures), credit_risk)
